...
| --- | --- | --- | --- | --- | --- | --- |
| TD-008 | High | Dashboard | `pragmatik dashboard` does not yet provide a real Pragmatik dashboard UI. | Users expect a visual dashboard for usage, tooling, coverage, and optimization but only status/report commands are available. | Design and implement a real local dashboard with session history, cost comparison, and time-savings metrics (v0.5.0). | [#10](https://github.com/mvuljevas/pragmatik/issues/10) |
